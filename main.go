...
// 	for i, artist := range ArtistsNew {
// 		if artist.ID == 21 {
// 			ArtistsNew = append(ArtistsNew[:i], ArtistsNew[i+1:]...)
// 			break
// 		}
// 	}
// }
...
func Func() {
	...
	var GroupieNew = Groupie{}
	if !Data(Url, &GroupieNew) {
		ArtistsNew = []Artists{{Result: false}}
		return
	}
	if !Data(GroupieNew.Artists, &ArtistsNew) {
		ArtistsNew = []Artists{{Result: false}}
...
func Data(url string, val interface{}) bool {
	res, err := http.Get(url)
	if err != nil {
		log.Printf("Error fetching data from %s: %s\n", url, err.Error())
		return false
	}
	...
	err = json.Unmarshal(body, &val)
	if err != nil {
		log.Printf("Error unmarshalling JSON data from %s: %s\n", url, err.Error())
		return false
	}
...
func index(w http.ResponseWriter, r *http.Request) {
	if !ArtistsNew[0].Result {
		Err("Internal Server Error", http.StatusInternalServerError, w, r)
		return
	}
	if r.Method != "GET" {
		Err("Method Not Allowed", http.StatusMethodNotAllowed, w, r)
		return
	}
	if r.URL.Path != "/" {
		Err("Not Found", http.StatusNotFound, w, r)
		return
	}
...
func artist(w http.ResponseWriter, r *http.Request) {
	...
	if len(r.URL.Path) < 10 || r.URL.Path[:9] != "/artists/" {
		Err("Bad Request", http.StatusBadRequest, w, r)
		return
	}
	if r.Method != "GET" {
		Err("Method Not Allowed", http.StatusMethodNotAllowed, w, r)
	...
	if err != nil || id < 1 || id > len(ArtistsNew) {
		Err("Bad Request", http.StatusBadRequest, w, r)
		return
	}
...
func Filter(w http.ResponseWriter, r *http.Request) {
	...
	if r.URL.Path != "/filters/" || r.Method != "GET" {
		Err("Bad Request", http.StatusBadRequest, w, r)
		return
	}
	...
	// Other filter criteria handling...
